...
object MovieLensALS {
  ...
  def main(args: Array[String]) {
    ...
    val model = ALS.train(train.union(myRating), 20, 10)
    ...
    if (baseline.size > 0) {
      val baselineRmse = rmse(test, baseline, sc)
  ...
  def loadRatings(path: String, sc: SparkContext) = {
    ...
    val baseline = ratingsData
      ...
      .map(x => (x._1, x._2._1 / x._2._2)) //(movieId, averageRating)
